...
bool WalletModel::validateAddress(const QString &address)
{
  ...
  // Validate the passed OdynStock address
  ...
  COdynStockAddress addressParsed(address_str);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, CAmount& total, const CCoinControl *coinControl, const CandidateTransaction* selectedCoins)
{
    ...
    for(const SendCoinsRecipient &rcp: recipients)
    {
        ...
        // User-entered odynstock address / amount:
        ...
        CTxDestination address = COdynStockAddress(rcp.address.toStdString()).Get();
...

    {
        ...
        for(const CRecipient &rcp: transaction.vecSend)
        {
          ...
          for(const CWalletTx newTx: transaction.vTransactions)
          {
            ...
            {
              if (!rcp.message.isEmpty()) // Message from normal odynstock:URI (odynstock:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(COdynStockAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for(const COutput& out: vCoins)
    {
        ...
        if (ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            sAddress = COdynStockAddress(address).ToString();
...
{
    CTxDestination dest = COdynStockAddress(sAddress).Get();
