...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus,
        OdynStockUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee(), fPrivate));
    ...
    if (currentTransaction.fSpendsColdStaking && (!fCoinControl ||
        (fCoinControl && !COdynStockAddress(CoinControlDialog::coinControl->destChange).IsColdStakingAddress(Params()))))
    {
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + OdynStockUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount - (rcp.fSubtractFeeFromAmount ? txFee : 0) , false, OdynStockUnits::SeparatorStyle::separatorStandard, false, rcp.isanon);
        amount.append("</b>");
        ...
        if(txFee)
        {
            ...
            questionString.append(OdynStockUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee, false, OdynStockUnits::SeparatorStyle::separatorStandard, false, rcp.isanon));
    ...
    if (model->getOptionsModel()->getDisplayUnit() != OdynStockUnits::BTC)
        alternativeUnits.append(OdynStockUnits::formatHtmlWithUnit(OdynStockUnits::BTC, totalAmount));

    questionString.append(tr("Total Amount %1")
        .arg(OdynStockUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(OdynStockUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    ...
    COdynStockAddress addr = COdynStockAddress(text.toStdString());
    ...
    {
        ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid OdynStock change address"));
