...
#include "odynstocklistwidget.h"

OdynStockListWidget::OdynStockListWidget(QWidget *parent, QString title, ValidatorFunc validator) :
    QWidget(parent),
...
void OdynStockListWidget::onRemove()
{
    listWidget->takeItem(listWidget->row(listWidget->currentItem()));
}

void OdynStockListWidget::onSelect(QListWidgetItem* item)
{
    removeBtn->setVisible(item != nullptr);
}

void OdynStockListWidget::onInsert()
{
...
QStringList OdynStockListWidget::getEntries()
{
    QStringList ret;
