    ...
    platformStyle(platformStyle)
{
    ...
    // see https://github.com/odynstock/odynstock/issues/5716
...
{
    GUIUtil::setClipboard(OdynStockUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog, CAmount& ccAmount)
{
    ...
    l2->setText(OdynStockUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(OdynStockUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(OdynStockUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(OdynStockUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && ((CoinControlDialog::fPrivate ? blscctCoinControl : coinControl)->nMinimumTotalFee < nPayFee))
    {
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(OdynStockUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000))) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(OdynStockUnits::formatHtmlWithUnit(nDisplayUnit, CWallet::GetRequiredFee(1000)));
...
void CoinControlDialog::updateView()
{
    ...
    for(const std::pair<QString, std::vector<COutput>>& coins: mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            {
                sAddress = QString::fromStdString(COdynStockAddress(outputAddress).ToString());

                // if listMode or change => show odynstock address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, OdynStockUnits::format(nDisplayUnit, out.tx->vout[out.i].HasRangeProof() ? out.nAmount : out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, OdynStockUnits::format(nDisplayUnit, nSum));
