...
const QString OdynStockGUI::DEFAULT_WALLET = "~Default";

OdynStockGUI::OdynStockGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    fShowingVoting(0)
{
    ...
    for(OdynStockUnits::Unit u: OdynStockUnits::availableUnits())
    {
        unitDisplayControl->addItem(QString(OdynStockUnits::name(u)), u);
    }
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &OdynStockGUI::showModalOverlay);
        connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &OdynStockGUI::showModalOverlay);
        connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &OdynStockGUI::showModalOverlay);
    }
...
float OdynStockGUI::scale()
{
    return GUIUtil::scale();
}

void OdynStockGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->Icon(":/icons/odynstock"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a OdynStock address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and odynstock: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your OdynStock addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->IconAlt(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified OdynStock addresses"));
    ...
    openAction->setStatusTip(tr("Open a odynstock: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->IconAlt(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible OdynStock command-line options").arg(tr(PACKAGE_NAME)));
...
void OdynStockGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void OdynStockGUI::createHeaderWidgets()
{
...
void OdynStockGUI::createToolBars()
{
...
void OdynStockGUI::showOutOfSyncWarning(bool fShow)
{
    showHideNotification(fShow, 1);
}

void OdynStockGUI::showHideNotification(bool show, int index)
{
    notifications[index]->setVisible(show);
}

void OdynStockGUI::setActiveMenu(int index)
{
    for (int i = 0; i < 6; ++i)
    {
...
bool OdynStockGUI::checkSettingsSaved()
{
...
void OdynStockGUI::setBalance(const CAmount &avail, const CAmount &pendi, const CAmount &immat, const CAmount& priv, const CAmount& privpending, const CAmount& privlocked)
{
    ...
    balanceAvail->setText(OdynStockUnits::prettyWithUnit(unit, avail, false, OdynStockUnits::separatorAlways));
    balancePendi->setText(OdynStockUnits::prettyWithUnit(unit, pendi, false, OdynStockUnits::separatorAlways));
    balanceImmat->setText(OdynStockUnits::prettyWithUnit(unit, immat, false, OdynStockUnits::separatorAlways));
    privAvail->setText(OdynStockUnits::prettyWithUnit(unit, priv, false, OdynStockUnits::separatorAlways, true));
    privPendi->setText(OdynStockUnits::prettyWithUnit(unit, privpending, false, OdynStockUnits::separatorAlways, true));
}

void OdynStockGUI::setStaked(const CAmount &all, const CAmount &today, const CAmount &week)
{
    if (!walletFrame || !clientModel || !clientModel->getOptionsModel())
        return;
    ...
    stakedAvail->setText(OdynStockUnits::prettyWithUnit(unit, all, false, OdynStockUnits::separatorAlways));
    stakedPendi->setText(OdynStockUnits::prettyWithUnit(unit, today, false, OdynStockUnits::separatorAlways));
    stakedImmat->setText(OdynStockUnits::prettyWithUnit(unit, week, false, OdynStockUnits::separatorAlways));
}

void OdynStockGUI::onDaoEntriesChanged(int count)
{
...
void OdynStockGUI::setMenuBubble(int index, int drak)
{
...
void OdynStockGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool OdynStockGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool OdynStockGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void OdynStockGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void OdynStockGUI::repairWallet()
{
...
void OdynStockGUI::updateAlerts(const QString &warnings)
{
...
void OdynStockGUI::setWalletActionsEnabled(bool enabled)
{
...
void OdynStockGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void OdynStockGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &OdynStockGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void OdynStockGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void OdynStockGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void OdynStockGUI::optionsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void OdynStockGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void OdynStockGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void OdynStockGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void OdynStockGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void OdynStockGUI::infoClicked()
{
    if(!clientModel)
        return;

    QString link = QString("https://info.odynstock.org/");
    QDesktopServices::openUrl(QUrl(link));
}

void OdynStockGUI::showDebugWindow()
{
...
void OdynStockGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void OdynStockGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void OdynStockGUI::openClicked()
{
...
void OdynStockGUI::gotoOverviewPage()
{
...
void OdynStockGUI::gotoHistoryPage()
{
    if (!checkSettingsSaved())
        return;
...
void OdynStockGUI::gotoCommunityFundPage()
{
    if (!checkSettingsSaved())
        return;
...
void OdynStockGUI::gotoSettingsPage()
{
    setActiveMenu(5);
...
void OdynStockGUI::gotoReceiveCoinsPage()
{
    if (!checkSettingsSaved())
        return;
...
void OdynStockGUI::gotoRequestPaymentPage()
{
    if (!checkSettingsSaved())
        return;
...
void OdynStockGUI::gotoSendCoinsPage(QString addr)
{
    if (!checkSettingsSaved())
        return;
...
void OdynStockGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void OdynStockGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void OdynStockGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to OdynStock network", "", count));
}

void OdynStockGUI::updateHeadersSyncProgressLabel()
{
...
void OdynStockGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
#ifdef Q_OS_MAC
...
void OdynStockGUI::showVotingDialog()
{
...
void OdynStockGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("OdynStock"); // default title
...
void OdynStockGUI::changeEvent(QEvent *e)
{
...
void OdynStockGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void OdynStockGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void OdynStockGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(OdynStockUnits::formatWithUnit(unit, amount, true, OdynStockUnits::SeparatorStyle::separatorStandard, type.contains("Private Payment"))) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void OdynStockGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void OdynStockGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool OdynStockGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool OdynStockGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void OdynStockGUI::setEncryptionStatus(int status)
{
...
void OdynStockGUI::setEncryptionTxStatus(bool fCrypted)
{
...
void OdynStockGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void OdynStockGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void OdynStockGUI::detectShutdown()
{
...
void OdynStockGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void OdynStockGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void OdynStockGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(OdynStockGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static std::string AskForPin(OdynStockGUI *gui)
{
    std::string ret = "";
...
void OdynStockGUI::askForPin(std::string *ret)
{
    bool ok = false;
...
void SetBalance(OdynStockGUI *gui, const CAmount& total, const CAmount& avail, const CAmount &immat, const CAmount& priv, const CAmount& privpending, const CAmount& privlocked)
{
    // Call our instance method
    gui->setBalance(total, avail, immat, priv, privpending, privlocked);
}

void SetStaked(OdynStockGUI *gui, const CAmount& all, const CAmount& today, const CAmount &week)
{
    // Call our instance method
    gui->setStaked(all, today, week);
}

void OdynStockGUI::subscribeToCoreSignals()
{
...
void OdynStockGUI::unsubscribeFromCoreSignals()
{
...
void OdynStockGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
...
void OdynStockGUI::updateDisplayUnit(int unit)
{
    // Update the list value
    unitDisplayControl->setCurrentText(OdynStockUnits::name(unit));
}

/** Update the display currency **/
void OdynStockGUI::comboBoxChanged(int index)
{
...
void OdynStockGUI::toggleStaking()
{
...
void OdynStockGUI::splitRewards()
{
...
void OdynStockGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
size_t OdynStockGUI::priceUdateWriteCallback(void* contents, size_t size, size_t nmemb, void* userp)
{
    ((std::string*) userp)->append((char*) contents, size * nmemb);
    return size * nmemb;
}

size_t OdynStockGUI::priceUdateWriteCallbackHeaders(void* contents, size_t size, size_t nitems, void* userdata)
{
    ((std::string*) userdata)->append((char*) contents, size * nitems);
    return size * nitems;
}

void OdynStockGUI::updateStakingStatus()
{
    updateWeight();
