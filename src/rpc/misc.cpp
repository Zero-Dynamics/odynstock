...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total odynstock balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            for(const CTxDestination& addr: addresses)
                a.push_back(COdynStockAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"odynstockaddress\"\n"
            "\nReturn information about the given odynstock address.\n"
            "\nArguments:\n"
            "1. \"odynstockaddress\"     (string, required) The odynstock address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"odynstockaddress\", (string) The odynstock address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"odynstockaddress\", (string) The odynstock staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"odynstockaddress\", (string) The odynstock spending address part of a cold staking address\n"
            "  \"votingaddress\" : \"odynstockaddress\", (string) The odynstock voting address part of a cold staking v2 address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            COdynStockAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            COdynStockAddress spendingAddress;
            ...
            COdynStockAddress spendingAddress;
            address.GetSpendingAddress(spendingAddress);
            COdynStockAddress votingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: OdynStock address and we have full public key:
        COdynStockAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are odynstock addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) odynstock address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    COdynStockAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    COdynStockAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"odynstockaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"odynstockaddress\"  (string, required) The odynstock address to use for the signature.\n"
...
{
    if (type == 2) {
        address = COdynStockAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = COdynStockAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            COdynStockAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
bool getAddressesFromParamsForHistory(const UniValue& params, std::vector<std::pair<std::pair<uint160, uint160>, AddressHistoryFilter>> &addresses)
{
    if (params[0].isStr()) {
        ...
        {
            COdynStockAddress addressVoting, addressStaking, addressSpending;
        ...
        {
            COdynStockAddress addressStaking, addressSpending;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            if (address.IsColdStakingv2Address(Params()))
            {
                COdynStockAddress addressSpending, addressVoting, addressStaking;
                uint160 hashBytes, hashBytesSpending, hashBytesVoting, hashBytesStaking;
                int type = 0;
...
UniValue getaddresshistory(const UniValue& params, bool fHelp)
{
    ...
    std::map<COdynStockAddress, balStruct> balance;
