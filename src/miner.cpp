        ...
        if(view.GetAllPaymentRequests(mapPaymentRequests))
        {
            for (CPaymentRequestMap::iterator it_ = mapPaymentRequests.begin(); it_ != mapPaymentRequests.end(); it_++)
            {
                ...
                if(prequest.GetLastState() == DAOFlags::ACCEPTED &&
                    ...
                    CProposal proposal;
                    if(view.GetProposal(prequest.proposalhash, proposal)) {
                        COdynStockAddress addr(proposal.GetPaymentAddress());
...
void OdynStockStaker(const CChainParams& chainparams)
{

    LogPrintf("OdynStockStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("odynstock-staker");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in OdynStockStaker: Keypool ran out, please call keypoolrefill before restarting the staking thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;

            //LogPrint("coinstake","Running OdynStockStaker with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("OdynStockStaker terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("OdynStockStaker runtime error: %s\n", e.what());
        return;
    }
}
...
bool CheckStake(CBlock* pblock, CWallet& wallet, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        {
            return error("OdynStockStaker: ProcessNewBlock, block not accepted");
        }
        else
        {
