...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CMutableTransaction& txNew, CKey& key, CScript& kernelScriptPubKey)
{
    ...
    std::map<COdynStockAddress, double> splitMap;
    double nAccumulatedFee = 0.0;

    COdynStockAddress poolFeeAddress(GetArg("-pooladdress", ""));
    ...
    else if (GetArg("-stakingaddress", "") != "")
    {
        ...
        if (stakingAddress.read(GetArg("-stakingaddress", "")))
        {
            try {
                ...
                if (find_value(addressMap, COdynStockAddress(key.GetPubKey().GetID()).ToString()).isStr())
                    lookForKey = COdynStockAddress(key.GetPubKey().GetID()).ToString();

                if (find_value(addressMap, COdynStockAddress(key.GetPubKey().GetID()).ToString()).isObject())
                {
                    find_value(addressMap, COdynStockAddress(key.GetPubKey().GetID()).ToString()).getObjMap(splitObject);
                ...
                {
                    address = COdynStockAddress(find_value(addressMap, lookForKey).get_str());
                ...
                else if(find_value(addressMap, lookForKey).isObject())
                {
                    ...
                    for ( const auto &pair : splitObject ) {
                        address = COdynStockAddress(pair.first);
        ...
        {
            address = COdynStockAddress(GetArg("-stakingaddress", ""));
            if (address.IsValid()) {
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = COdynStockAddress(CScriptID(redeemScript)).ToString();
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                        ...
                        (!coinControl || !coinControl->HasSelected() || coinControl->fAllowOtherInputs || coinControl->IsSelected(COutPoint((*it).first, i))))
                {
                    ...
                    if (pwalletMain->GetBLSCTSubAddressPublicKeys(pcoin->vout[i].outputKey, pcoin->vout[i].spendingKey, k))
                        sAddress = COdynStockAddress(k).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                else if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-odynstock-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(COdynStockAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(COdynStockAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = COdynStockAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(COdynStockAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(COdynStockAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(COdynStockAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(COdynStockAddress(dest).ToString(), key);
}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-stakingaddress", strprintf(_("Specify a customised odynstock address to accumulate the staking rewards.")));
