...
UniValue createrawscriptaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
                ...
                "\nReturns the OdynStock address for the specified raw hex script.\n"
                "\nArguments:\n"
                "1. \"hex script\"        (string) The hex script to encode in the address.\n"
                "\nResult:\n"
                "\"odynstockaddress\"    (string) The  odynstock address\n"
                "\nExamples:\n"
    ...
    COdynStockAddress address(script);
...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturns a new OdynStock address for receiving payments.\n"
                "If 'account' is specified (DEPRECATED), it is added to the address book \n"
                "so payments received with the address will be credited to 'account'.\n"
                "\nArguments:\n"
                "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
                "\nResult:\n"
                "\"odynstockaddress\"    (string) The new odynstock address\n"
                "\nExamples:\n"
    ...
    return COdynStockAddress(keyID).ToString();
}
...
UniValue getcoldstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw std::runtime_error(
            ...
            "Arguments:\n"
            "1. \"stakingaddress\"  (string, required) The odynstock staking address.\n"
            "2. \"spendingaddress\" (string, required) The odynstock spending address.\n\n"
            "3. \"voting\"          (string, optional) The odynstock voting address.\n\n"
            "\nExamples:\n"
    ...
    COdynStockAddress stakingAddress(params[0].get_str());
    CKeyID stakingKeyID;
    if (!stakingAddress.IsValid() || !stakingAddress.GetKeyID(stakingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Staking address is not a valid OdynStock address");

    COdynStockAddress spendingAddress(params[1].get_str());
    CKeyID spendingKeyID;
    if (!spendingAddress.IsValid() || !spendingAddress.GetKeyID(spendingKeyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Spending address is not a valid OdynStock address");

    spendingAddress.GetKeyID(spendingKeyID);

    if (params.size() == 3)
    {
        COdynStockAddress votingAddress(params[2].get_str());
        CKeyID votingKeyID;
        if (!votingAddress.IsValid() || !votingAddress.GetKeyID(votingKeyID))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Voting address is not a valid OdynStock address");

        votingAddress.GetKeyID(votingKeyID);

        return COdynStockAddress(stakingKeyID, spendingKeyID, votingKeyID).ToString();
    }

    return COdynStockAddress(stakingKeyID, spendingKeyID).ToString();
}


COdynStockAddress GetAccountAddress(std::string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return COdynStockAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
                ...
                "\nDEPRECATED. Returns the current OdynStock address for receiving payments to this account.\n"
                "\nArguments:\n"
                "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
                "\nResult:\n"
                "\"odynstockaddress\"   (string) The account odynstock address\n"
                "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
                "getrawchangeaddress\n"
                "\nReturns a new OdynStock address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
                "setaccount \"odynstockaddress\" \"account\"\n"
                "\nDEPRECATED. Sets the account associated with the given address.\n"
                "\nArguments:\n"
                "1. \"odynstockaddress\"  (string, required) The odynstock address to be associated with an account.\n"
    ...
    COdynStockAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OdynStock address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
                "getaccount \"odynstockaddress\"\n"
                "\nDEPRECATED. Returns the account associated with the given address.\n"
                "\nArguments:\n"
                "1. \"odynstockaddress\"  (string, required) The odynstock address for account lookup.\n"
                "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
                ...
                "  \"odynstockaddress\"  (string) a odynstock address associated with the given account\n"
                "  ,...\n"
                "]\n"
                "\nExamples:\n"
    ...
    for(const std::pair<COdynStockAddress, CAddressBookData>& item: pwalletMain->mapAddressBook)
    {
        const COdynStockAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
                "sendtoaddress \"odynstockaddress\" amount ( \"comment\" \"comment-to\" \"strdzeel\" subtractfeefromamount )\n"
                "\nSend an amount to a given address.\n"
                + HelpRequiringPassphrase() +
                "\nArguments:\n"
                "1. \"odynstockaddress\"  (string, required) The odynstock address to send to.\n"
                "2. \"amount\"      (numeric or string, required) The amount in " + CURRENCY_UNIT + " to send. eg 0.1\n"
                                                                                                    ...
                                                                                                    "                             The recipient will receive less odynstocks than you enter in the amount field.\n"
                                                                                                    "\nResult:\n"
    ...
    COdynStockAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OdynStock address");
...
UniValue privatesendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
                "privatesendtoaddress \"odynstockaddress\" amount \"comment\" ( \"comment-to\" \"strdzeel\" subtractfeefromamount )\n"
...
UniValue privatesendmixtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
                "privatesendmixtoaddress \"odynstockaddress\" amount \"comment\" ( \"comment-to\" \"strdzeel\" subtractfeefromamount )\n"
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (!paddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OdynStock address for payment");

    COdynStockAddress oaddress(ownerAddress);
    if (!oaddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OdynStock address for the owner");
...
UniValue createconsultation(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1)
        throw std::runtime_error(
            ...
            "\"  strDZeel: string }\"            (string) The attached strdzeel property.\n"
            "\nExamples:\n"
            + HelpExampleCli("createconsultation", "\"Who should be the CEO of OdynStock? /s\" 1 1")
                + HelpExampleCli("createconsultation", "\"How much should OdynStock's CEO earn per month? /s\" 1000 5000 true")
                );
...
UniValue createconsultationwithanswers(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2)
        throw std::runtime_error(
            ...
            "\"  strDZeel: string }\"            (string) The attached strdzeel property.\n"
            "\nExamples:\n"
            + HelpExampleCli("createconsultationwithanswers", "\"Who should be the CEO of OdynStock? /s\" \"[\\\"Craig Wright\\\",\\\"Loomdart\\\"]\"")
                );
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid OdynStock address.");
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
                ...
                "1. \"amount\"      (numeric or string, required) The amount in " + CURRENCY_UNIT + " to donate. eg 0.1\n"
                                                                                                    ...
                                                                                                    "                             The fund will receive less odynstocks than you enter in the amount field.\n"
                                                                                                    "\nResult:\n"
                                                                                                    "\"transactionid\"  (string) The transaction id.\n"
                                                                                                    "\nExamples:\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    for(std::set<CTxDestination> grouping: pwalletMain->GetAddressGroupings())
    {
        ...
        for(CTxDestination address: grouping)
        {
            ...
            addressInfo.push_back(COdynStockAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(COdynStockAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(COdynStockAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
                "signmessage \"odynstockaddress\" \"message\"\n"
                "\nSign a message with the private key of an address"
                + HelpRequiringPassphrase() + "\n"
                                              "\nArguments:\n"
                                              "1. \"odynstockaddress\"  (string, required) The odynstock address to use for the private key.\n"
    ...
    COdynStockAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
                "getreceivedbyaddress \"odynstockaddress\" ( minconf )\n"
                "\nReturns the total amount received by the given odynstockaddress in transactions with at least minconf confirmations.\n"
                "\nArguments:\n"
                "1. \"odynstockaddress\"  (string, required) The odynstock address for transactions.\n"
    ...
    // OdynStock address
    COdynStockAddress address = COdynStockAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OdynStock address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
                "sendfrom \"fromaccount\" \"toodynstockaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
                "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a odynstock address."
                + HelpRequiringPassphrase() + "\n"
                                              "\nArguments:\n"
                                              "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                                              "2. \"toodynstockaddress\"  (string, required) The odynstock address to send funds to.\n"
    ...
    std::string strAccount = AccountFromValue(params[0]);
    COdynStockAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OdynStock address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
                ...
                + HelpRequiringPassphrase() + "\n"
                                              ...
                                              "    {\n"
                                              "      \"address\":amount   (numeric or string) The odynstock address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
                                                                                                                                                                                        "      ,...\n"
                                                                                                                                                                                        "    }\n"
                                                                                                                                                                                        ...
                                                                                                                                                                                        "                           Those recipients will receive less odynstocks than you enter in their corresponding amount field.\n"
    ...
    std::set<COdynStockAddress> setAddress;
    std::vector<CRecipient> vecSend;
    ...
    {
        COdynStockAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid OdynStock address: ")+name_);
...
UniValue getnewprivateaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
                ...
                "\nReturns a new OdynStock private address for receiving payments.\n"
    ...
    pwalletMain->SetPrivateAddressBook(COdynStockAddress(k).ToString(), "blsct receive", std::to_string(index.first) + "/" + std::to_string(index.second));

    return COdynStockAddress(k).ToString();
}
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "Each key is a OdynStock address or hex-encoded public key.\n"
                     "If 'account' is specified (DEPRECATED), assign address to that account.\n"

                     "\nArguments:\n"
                     "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
                     "2. \"keysobject\"   (string, required) A json array of odynstock addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"address\"  (string) odynstock address or hex-encoded public key\n"
                     "       ...,\n"
                     "     ]\n"
                     "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

                     "\nResult:\n"
                     "\"odynstockaddress\"  (string) A odynstock address associated with the keys.\n"
    ...
    return COdynStockAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return COdynStockAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    std::map<COdynStockAddress, tallyitem> mapTally;
    for (std::map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    for(const std::pair<COdynStockAddress, CAddressBookData>& item: pwalletMain->mapAddressBook)
    {
        ...
        std::map<COdynStockAddress, tallyitem>::iterator it = mapTally.find(address);
    ...
    if (fAllAccounts || (account == strAccount))
    {
        ...
        entry.pushKV("canStake", (::IsMine(*pwalletMain, r.destination) & ISMINE_STAKABLE ||
                                  (::IsMine(*pwalletMain, r.destination) & ISMINE_SPENDABLE &&
                                   !COdynStockAddress(r.destination).IsColdStakingAddress(Params()) &&
                                   !COdynStockAddress(r.destination).IsColdStakingv2Address(Params()))) ? true : false);
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw std::runtime_error(
                ...
                "    \"address\":\"odynstockaddress\",    (string) The odynstock address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw std::runtime_error(
                ...
                "    \"address\":\"odynstockaddress\",    (string) The odynstock address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
                ...
                "      \"address\" : \"odynstockaddress\",   (string) The odynstock address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2))
        throw std::runtime_error(
                ...
                "This is needed prior to performing transactions related to private keys such as sending odynstocks\n"
                "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
                ...
                "\nNow set the passphrase to use the wallet, such as for signing or sending odynstock\n"
                + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
                "\nNow we can so something like sign\n"
                + HelpExampleCli("signmessage", "\"odynstockaddress\" \"test message\"") +
    ...
    StartShutdown();
    return _("wallet encrypted; OdynStock server stopping, restart to run with encrypted wallet.");
}
...
UniValue encrypttxdata(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return _("txdata encrypted; OdynStock server stopping, restart to run with encrypted txdata.");
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending odynstocks.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
                ...
                "1. minconf          (numeric, optional, default=1) The minimum confirmations to filter\n"
                "2. maxconf          (numeric, optional, default=9999999) The maximum confirmations to filter\n"
                "3. \"addresses\"    (string) A json array of odynstock addresses to filter\n"
                "    [\n"
                "      \"address\"   (string) odynstock address\n"
                "      ,...\n"
                "    ]\n"
                "\nResult\n"
                "[                   (array of json object)\n"
                ...
                "    \"txid\" : \"txid\",          (string) the transaction id \n"
                "    \"vout\" : n,               (numeric) the vout value\n"
                "    \"address\" : \"address\",    (string) the odynstock address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            COdynStockAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid OdynStock address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    for(const COutput& out: vecOutputs) {
        ...
        if (fValidAddress) {
            entry.pushKV("address", COdynStockAddress(address).ToString());
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
                ...
                "   {\n"
                "     \"changeAddress\"     (string, optional, default pool address) The odynstock address to receive the change\n"
    ...
    if (params.size() > 1) {
        if (params[1].type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress")) {
                COdynStockAddress address(options["changeAddress"].get_str());

                if (!address.IsValid())
                    throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid odynstock address");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
    ...
    if ((fHelp || params.size() != 1))
        throw std::runtime_error(
                ...
                "\nResolves the given OpenAlias address to a OdynStock address.\n"
                "\nArguments:\n"
                "1. \"address\"    (string) The OpenAlias address.\n"
                "\nExamples:\n"
                "\nGet information about an OpenAlias address\n"
                + HelpExampleCli("resolveopenalias", "\"donate@odynstock.org\"")
                );
...
UniValue listproposals(const UniValue& params, bool fHelp)
{
    ...
    if(view.GetAllProposals(mapProposals))
    {
        for (CProposalMap::iterator it = mapProposals.begin(); it != mapProposals.end(); it++)
        {
            ...
            if (showMine)
            {
                CTxDestination address(COdynStockAddress(proposal.GetOwnerAddress()).Get());
                isminefilter mine = IsMine(*pwalletMain, address);
