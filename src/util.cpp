...
const char * const ODYNSTOCK_CONF_FILENAME = "odynstock.conf";
const char * const ODYNSTOCK_PID_FILENAME = "odynstock.pid";
...
#else
    const char* pszModule = "odynstock";
#endif
    if (pex)
        return strprintf(
...
fs::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OdynStock
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OdynStock
    // Mac: ~/Library/Application Support/OdynStock
    // Unix: ~/.odynstock
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OdynStock";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OdynStock";
#else
    // Unix
    return pathRet / ".odynstock";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", ODYNSTOCK_CONF_FILENAME));
                    ...
                    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!stream.good())
        return; // No odynstock.conf file is OK
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", ODYNSTOCK_PID_FILENAME));
