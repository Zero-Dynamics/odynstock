...
*Setup instructions for a Gitian build of OdynStock Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the OdynStock
...
- [Building OdynStock Core](#building-odynstock-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-odynstock-host/README.md).
...
Clone the git repositories for odynstock and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/odynstock/odynstock-core
```
...
in the odynstock repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building OdynStock Core
----------------

To build OdynStock Core (for Linux, OS X and Windows) just follow the steps under 'Setup and perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the odynstock repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/odynstock/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/odynstock/odynstock-core
...
For example:
```bash
URL=https://github.com/odynstock/odynstock-core.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit odynstock-core=${COMMIT} --url odynstock-core=${URL} ../odynstock-core/contrib/gitian-descriptors/gitian-arm.yml
./bin/gbuild --commit odynstock-core=${COMMIT} --url odynstock-core=${URL} ../odynstock-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit odynstock-core=${COMMIT} --url odynstock-core=${URL} ../odynstock-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit odynstock-core=${COMMIT} --url odynstock-core=${URL} ../odynstock-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the odynstock git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../odynstock/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/odynstock-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/odynstock-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/odynstock-osx-build.assert
```
...
[OdynStock/odynstock-sigs](https://github.com/odynstock/odynstock-sigs) repository, or if that's not possible create a pull
request.
