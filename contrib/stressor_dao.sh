...
function nav_cli {
	$navpath/odynstock-cli -datadir=${array_data[$1]} -rpcport=${array_rpc_port[$1]} -devnet $2 2> /dev/null
}
...
function start_node {
	$(echo $navpath)/odynstockd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -debug=dao -debug=daoextra -debug=statehash -ntpminmeasures=-1 -dandelion=0 -disablesafemode -staking=0 -daemon
#	gdb -batch -ex "run" -ex "bt" --args $(echo $navpath)/odynstockd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -debug=daoextra -debug=dao -debug=statehash -ntpminmeasures=-1 -dandelion=0 -disablesafemode -staking=0 &
}
