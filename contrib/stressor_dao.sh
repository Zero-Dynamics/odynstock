...
function odyns_cli {
	$odynspath/odynstock-cli -datadir=${array_data[$1]} -rpcport=${array_rpc_port[$1]} -devnet $2 2> /dev/null
}
...
function terminate {
	...
	then
		...
		do
			...
			odyns_cli $i listproposals > ${array_data[$i]}/devnet/listproposals.out
			odyns_cli $i listconsultations > ${array_data[$i]}/devnet/listconsultations.out
...
function connect_nodes {
	odyns_cli "$1" "addnode 127.0.0.1:$( bc <<< 10000+$2 ) add"
	odyns_cli "$1" "addnode 127.0.0.1:$( bc <<< 10000+$2 ) onetry"
}

function disconnect_nodes {
	odyns_cli "$1" "addnode 127.0.0.1:$( bc <<< 10000+$2 ) remove"
	odyns_cli "$1" "disconnectnode 127.0.0.1:$( bc <<< 10000+$2 )"
}
...
function check_connection {
	...
	do
		...
		check=$(odyns_cli ${connection_pair[0]} getpeerinfo | grep "\"addr\": \"127.0.0.1:$( bc <<< "10000+${connection_pair[1]}" )\"")
		if [ -z "$check" ];
		then
...
function start_random_stopped_nodes {
	...
	then
		...
		do
			...
			echo "Nodes $j balance: $(odyns_cli $j getbalance) t0DYNS"
		done
	fi
	echo Currently active nodes are: ${array_active_nodes[@]} inactive ones are: ${array_stopped_nodes[@]}
}
...
function wait_until_sync {
	...
	for i in ${local_array[@]};
	do
		local_array_best_hash[$i]=$(odyns_cli $i getbestblockhash)
#		echo best block hash of node $i:
#		echo "$(odyns_cli $i getbestblockhash)"
	done
...
#		echo array topology node pairs stopped is "${array_topology_node_pairs_stopped[@]}"
		...
		$(odyns_cli $node "generate 1")
		echo now on blcok $(odyns_cli $node "getblockcount")
...
function assert_state {
	...
	for i in ${local_array[@]};
	do
		local_array_statehash[$i]=$(odyns_cli $i getcfunddbstatehash)
	done
...
function random_transactions {
	...
	if [ $dice -lt 75 ];
		...
		out=$(odyns_cli ${array_stressing_nodes[$node_send]} "sendtoaddress ${array_address[$node_receive]} $transaction_amount")
	fi
}
...
function dice_proposal {
	...
	then
        	...
        	then
        		...
        		address=$(odyns_cli ${array_stressing_nodes[$node]} getnewaddress)
        		out=$(odyns_cli ${array_stressing_nodes[$node]} "createproposal $address $amount $deadline \"$random_sentence\"")
		else
        		...
        		deadline=$(bc <<< "$RANDOM % 1000000")
        		address=$(odyns_cli ${array_stressing_nodes[$node]} getnewaddress)
        		out=$(odyns_cli ${array_stressing_nodes[$node]} "createproposal $address $amount $deadline \"$random_sentence\" 50 false $address true")
        	fi
	fi
	...
	then
		proposals=$(odyns_cli ${array_stressing_nodes[$node]} "listproposals mine"|jq -r ".[]|.hash"|tr "\n" " ")
		array_proposals=($proposals)
		for p in ${array_proposals[@]}
		do
			proposal=$(odyns_cli $node "getproposal $p")
			...
			then
				...
				then
					...
					out=$(odyns_cli ${array_stressing_nodes[$node]} "createpaymentrequest $hash $requestAmount \"$random_sentence\"")
				fi
			fi
		done
	fi
...
function dice_consultation {
	...
	then
		...
		out=$(odyns_cli ${array_stressing_nodes[$node]} "createconsultation $random_sentence $random_max_answer")
	fi
	...
	then
		...
		out=$(odyns_cli ${array_stressing_nodes[$node]} "createconsultationwithanswers $random_sentence [\"$consultation_answer\"] $random_max_answer $bool_user_propose_new_answer")
	fi
	...
	then
		...
		out=$(odyns_cli ${array_stressing_nodes[$node]} "createconsultation $random_sentence $random_lower_limit $random_upper_limit true")
	fi
	...
	then
		...
		out=$(odyns_cli ${array_stressing_nodes[$node]} "proposeconsensuschange $consensus $value")
...
#		echo "changing consensus $changing_consensus with values $changing_consensus_value"
		out=$(odyns_cli ${array_stressing_nodes[$node]} "proposecombinedconsensuschange [$changing_consensus] [$changing_consensus_value]")
#		echo "out = $out"
#		echo "Proposing changing consensus ${consensus_parameter_name[$consensus]} from ${consensusparameter_new[$consensus]} to $value"
	fi
	...
	then
		consultations=($(odyns_cli ${array_stressing_nodes[$node]} "listconsultations"|jq -r ".[]|.hash"|tr "\n" " "))
		for c in ${consultations[@]}
		do
			consultation=$(odyns_cli $node "getconsultation $c")
				...
				then
					...
					do
						...
						then
							...
							out=$(odyns_cli ${array_stressing_nodes[$node]} "proposeanswer $hash $consensus $value")
#							echo Adding $value as a new answer to $question
				...
				then
					random_sentence=$(env LC_CTYPE=C tr -dc "a-zA-Z0-9-_\$\?" < /dev/urandom | head -c 10)
					out=$(odyns_cli ${array_stressing_nodes[$node]} "proposeanswer $hash \"$random_sentence\"")
				fi
			fi
		done
	fi
...
function voter_dice_proposal {
	...
	proposals=($(odyns_cli ${array_stressing_nodes[$node]} proposalvotelist|jq -r ".null[]|.hash"))
	for i in ${proposals[@]};
	do
		dice=$(bc <<< "$RANDOM % 2")
		if [ $dice -eq 1 ];
		then
			out=$(odyns_cli ${array_stressing_nodes[$node]} "proposalvote $i yes")
		else
			out=$(odyns_cli ${array_stressing_nodes[$node]} "proposalvote $i no")
		fi
	done

	prequests=($(odyns_cli ${array_stressing_nodes[$node]} paymentrequestvotelist|jq -r ".null[]|.hash?"))
	...
	do
		...
		if [ $dice -eq 1 ];
		then
			out=$(odyns_cli ${array_stressing_nodes[$node]} "paymentrequestvote $i yes")
		else
			out=$(odyns_cli ${array_stressing_nodes[$node]} "paymentrequestvote $i no")
		fi
	done
}
...
function voter_dice_consultation {
	...
	node=${shuffled_array[0]}
	consultations=($(odyns_cli ${array_stressing_nodes[$node]} "listconsultations"|jq -r ".[]|.hash"|tr "\n" " "))
	all_consultation_answers=($(odyns_cli ${array_stressing_nodes[$node]} "listconsultations"|jq -r ".[].answers[]|.hash"|tr "\n" " "))
	for i in ${consultations[@]};
	do
		consultation=$(odyns_cli $node "getconsultation $i")
		version=$(echo $consultation | jq -r .version)
		status=$(echo $consultation | jq -r .status)
		if [[ "$status" == "waiting for support" ]] || [[ "$status" == "waiting for support, waiting for having enough supported answers" ]];
		then
			...
			then
				...
				if [ $dice -eq 1 ];
				then
					out=$(odyns_cli ${array_stressing_nodes[$node]} "support $i")
				else
					out=$(odyns_cli ${array_stressing_nodes[$node]} "support $i false")
				fi
			else
				...
				do
					...
					if [ $dice -eq 0 ];
					then
						out=$(odyns_cli ${array_stressing_nodes[$node]} "support $k")
					else
						out=$(odyns_cli ${array_stressing_nodes[$node]} "support $k false")
					fi
				done
			fi
		...
		then
			...
			then
				...
				if [ $dice -eq 1 ];
				then
					out=$(odyns_cli ${array_stressing_nodes[$node]} "consultationvote $i $RANDOM")
				else
					out=$(odyns_cli ${array_stressing_nodes[$node]} "consultationvote $i abs")
				fi
			else
				out=$(odyns_cli ${array_stressing_nodes[$node]} "consultationvote $i remove")
				consultation_answers=($(odyns_cli $node "getconsultation $i"| jq -r ".answers[].hash" | tr "\n" " "))
				for k in ${consultation_answers[@]};
				do
					out=$(odyns_cli ${array_stressing_nodes[$node]} "consultationvote $k remove")
				done
				...
				then
					...
					then
						...
						out=$(odyns_cli ${array_stressing_nodes[$node]} "consultationvote ${consultation_answers[$yes_answer]} yes")
					fi
				else
					dice=$(bc <<< "$RANDOM % 5")
					if [ $dice -eq 1 ];
					then
						...
						do
							...
							if [ $dice -eq 1 ];
							then
								out=$(odyns_cli ${array_stressing_nodes[$node]} "consultationvote $k yes")
							fi
						done
					fi
				fi
...
function check_consensus_parameters {
	consensusparameter_tmp=($(odyns_cli $1 getconsensusparameters | tr -d "[],\n"))
...
function stress {
	...
	out=$(odyns_cli 0 "donatefund $donation")
	for i in ${array_stressing_nodes[@]};
	do
		out=$(odyns_cli $i "staking false")
		echo "Node $i balance: $(odyns_cli $i getbalance) t0DYNS"
	done
}

function check_node {
	blocks=`odyns_cli $2 getinfo|jq .blocks`
	block_increment=${#array_verifychain_nodes[@]}
	for i in $(seq $1 $block_increment $blocks);
	do
		verifyoutput=`odyns_cli $2 verifychain 4 $i`
	...
	verifyoutput=`odyns_cli $2 verifychain 4 0`
...
function check_cycle {
	shuffle_array "${array_stressing_nodes[@]}"
	node=${shuffled_array[0]}
	blocks=$(odyns_cli $node getinfo|jq .blocks)
...
function start_node {
	$(echo $odynspath)/odynstockd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -debug=dao -debug=daoextra -debug=statehash -ntpminmeasures=-1 -dandelion=0 -disablesafemode -staking=0 -daemon
#	gdb -batch -ex "run" -ex "bt" --args $(echo $odynspath)/odynstockd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -debug=daoextra -debug=dao -debug=statehash -ntpminmeasures=-1 -dandelion=0 -disablesafemode -staking=0 &
}

function stop_node {
	out=$(odyns_cli $1 stop)
}
...
#Sleep to let odynstockd boot up, on slower systems the value may need to be much higher
sleep 30
...
out=$(odyns_cli 0 "generate 10")

#Distribute funds to stressing nodes
array_address=()
for n in ${!array_stressing_nodes[@]};
do
        array_address[$n]=$(odyns_cli ${array_stressing_nodes[$n]} getnewaddress)
done
for i in {1..50};
do
	for j in ${array_address[@]};
	do
		out=$(odyns_cli 0 "sendtoaddress $j 10000")
	done
	out=$(odyns_cli 0 "generate 1")
done
sleep 1
for n in ${array_stressing_nodes[@]};
do
	echo "Node $n balance: $(odyns_cli $n getbalance) t0DYNS"
done

#Create blocks to make block count greater than 300
blocks=$(odyns_cli 0 getblockcount)
while [ $blocks -lt 300 ];
do
	out=$(odyns_cli 0 "generate 10")
	blocks=$(odyns_cli 0 getblockcount)
...
consensus_parameter_count=$(odyns_cli 0 getconsensusparameters | jq length)
for i in $(seq 0 1 $( bc <<< "$consensus_parameter_count - 1" ));
do
	eval "consensus_parameter_name[\$i]=\$(odyns_cli 0 \"getconsensusparameters true\" | jq -r '.[] | select(.id==$i) | .desc')"
done
consensusparameter_old=($(odyns_cli 0 getconsensusparameters | tr -d "[],\n"))
consensusparameter_original=("${consensusparameter_old[@]}")
voting_cycle_length=${consensusparameter_old[0]}
blocks=$(odyns_cli 0 getinfo|jq .blocks)
...
current_block=$(odyns_cli 0 getinfo|jq .blocks)
...
while [ $wait_until_cycle -gt $this_cycle ]; do
	...
	echo Proposals: $(odyns_cli $node listproposals|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
	echo Payment Requests: $(odyns_cli $node listproposals|jq -c "[.[].paymentRequests|map({status:.status})]|flatten|group_by(.status)|map({status:.[0].status,count:length})|.[]")
	echo Super Proposals: $(odyns_cli $node listproposals | jq '[.[] | select(.super_proposal == true)] | length')
	echo Consultations: $(odyns_cli $node listconsultations|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
	echo Consensus Parameter Change Proposed: $(odyns_cli $node listconsultations  | jq ".[].version" | grep 29 | wc -l)
	echo Combined Consensus Parameter Change Proposed: $(odyns_cli $node listconsultations  | jq ".[].version" | grep 61 | wc -l)
...
then
	...
	while [ $wait_until_cycle -gt $this_cycle ]; do
		...
		do
			...
			echo Payment Requests: $(odyns_cli $node listproposals|jq -c "[.[].paymentRequests|map({status:.status})]|flatten|group_by(.status)|map({status:.[0].status,count:length})|.[]")
			echo Consultations: $(odyns_cli $node listconsultations|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
			echo Consensus Parameter Change Proposed: $(odyns_cli $node listconsultations  | jq ".[].version" | grep 13 | wc -l)
		done
		cycles_left=$(bc <<< "$wait_until_cycle - $this_cycle")
		previous_block=$current_block
		current_block=$blocks
...
echo Waiting until all nodes are synced
wait_until_sync "${array_active_nodes[@]}"
echo Ok! Block: $(odyns_cli $node getinfo|jq .blocks) Cycle: $(bc <<< $(odyns_cli $node getinfo|jq .blocks)/$voting_cycle_length).
echo Proposals: $(odyns_cli $node listproposals|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
echo Payment Requests: $(odyns_cli $node listproposals|jq -c "[.[].paymentRequests|map({status:.status})]|flatten|group_by(.status)|map({status:.[0].status,count:length})|.[]")
echo Consultations: $(odyns_cli $node listconsultations|jq -c "map({status:.status})|group_by(.status)|map({status:.[0].status,count:length})|.[]")
