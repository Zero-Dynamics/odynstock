...
#		random_symbol=$(env LC_CTYPE=C tr -dc "a-zA-Z0-9-_\$\?" < /dev/urandom | head -c 3)
		out=$(odyns_cli $node registername $random_name.0dyns)
		if [ ! -z $out ]
		then
			echo 'create name success!'
			array_dotodynsnames+=("$random_name.0dyns")
			echo 'dotodynsnames= ' ${array_dotodynsnames[@]}
		fi
	fi
}
...
function dice_update_name {
	...
	then
		...
		for name in ${array_dotodynsnames[@]}
		do
			...
			if [ $(bc <<< "$RANDOM % 3") -lt 2 ];
			then
				out=$(odyns_cli $node updatename $name $random_key $random_value)
				if [ ! -z $out ]
				then
					echo 'update name success!'
				fi
			else
				out=$(odyns_cli $node updatename $random_subdomain.$name $random_key $random_value)
				if [ ! -z $out ]
				then
					echo 'update subdomain name success!'
					array_dotodynsnames+=("$random_subdomain.$name")
					echo 'dotodynsnames= ' ${array_dotodynsnames[@]}
				fi
			fi
		done
...
function dice_renew_name {
	dice=$(bc <<< "$RANDOM % 100")
		...
		shuffle_array "${array_dotodynsnames[@]}"
		name=${shuffled_array[0]}
	        out=$(odyns_cli $node renewname $name)
		if [ ! -z $out ]
		then
...
function check_dotodyns_hash {
		...
		do
			...
			echo $(odyns_cli $i resolvename $name)
			local_array_namehash+=($(odyns_cli $i resolvename $name | jq -c |shasum|awk '{print $1}'))
		done
		local_array_tokenhash[i]=$(echo ${local_array_namehash[@]} | shasum|awk '{print $1}')
		echo best block hash of node $i:
		echo "$(odyns_cli $i getbestblockhash)"
        done
	echo 'all registered names=' ${array_dotodynsnames[@]}
	echo 'dot 0dyns hashes = ' ${local_array_tokenhash[@]}
        ...
        then
                ...
                then
                        echo DOTODYNS HASH MISMATCH! Syncing again to make sure best block hashes match.
                        bool_assert_state_mismatch=1
                        wait_until_sync "${local_array[@]}"
                        check_dotodyns_hash "${local_array[@]}"
                else
                        echo DOTODYNS HASH MISMATCH!
                        for i in ${!local_array_tokenhash[@]};
                        do
                                echo the dotodyns hashes of nodes $i are ${local_array_tokenhash[$i]}
                        done
                        terminate 1
                fi
        fi
        echo DotOdyns hash check OK!
...
function start_node {
        $(echo $odynspath)/odynstockd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -daemon -debug=dao -debug=statehash -ntpminmeasures=-1 -dandelion=0 -disablesafemode -staking=0 2> /dev/null
#       gdb -batch -ex "run" -ex "bt" --args $(echo $odynspath)/odynstockd -datadir=${array_data[$1]} -port=${array_p2p_port[$1]} -rpcport=${array_rpc_port[$1]} -devnet -debug=dao -debug=statehash -ntpminmeasures=0 -dandelion=0 -disablesafemode -staking=0 > out.gdb &
}
...
while [ $wait_until_cycle -gt $this_cycle ]; do
	...
	check_dotodyns_hash "${array_active_nodes[@]}"
	shuffle_array "${array_stressing_nodes[@]}"
	node=${shuffled_array[0]}
	check_consensus_parameters $node
	...
	echo Tokens and NFTs created: $(odyns_cli $node listtokens  | jq ".[].name" |  wc -l)
...
then
	...
	while [ $wait_until_cycle -gt $this_cycle ]; do
		...
		do
			...
			echo Checking token hashes match...
			eval "check_dotodyns_hash \"\${array_all_nodes_network$nc[@]}\""
	...
	check_dotodyns_hash "${array_all_nodes[@]}"
fi
#############################
