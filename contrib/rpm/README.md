...
The RPM spec file provided here is for OdynStock-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://odynstock.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[OdynStock git repository](https://github.com/odynstock/odynstock) but are not part of
...
Currently only `Source10` is used, for the example `odynstock.conf` file.
...
in the odynstock git repository. At present only an SVG version of the OdynStock
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the odynstock
...
needed when 0.12.1 is released, a proper fix is already in the OdynStock git
master branch.
...
idea to build OdynStock Core against a static version of that library compiled
...
The `odynstock.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `odynstock-server` package. At this point, most RPM based
...
The `odynstock-server` package should have SELinux support. How to properly do
...
OdynStock-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/odynstock/). He has been
packaging OdynStock for Fedora at least since 2012.
...
4. I renamed the `odynstock` package that contains the Qt GUI to `odynstock-core` as
that appears to be how the general population refers to it, in contrast to
`odynstock-xt` or `odynstock-classic`. I wanted to make sure the general population
