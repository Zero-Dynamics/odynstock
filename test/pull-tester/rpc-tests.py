...
`test/pull-tester/test_framework/test_framework.py:OdynStockTestFramework.main`.
...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_ODYNSTOCKD' not in vars():
    ENABLE_ODYNSTOCKD=0
...
#Set env vars
if "ODYNSTOCKD" not in os.environ:
    os.environ["ODYNSTOCKD"] = BUILDDIR + '/src/odynstockd' + EXEEXT
if "ODYNSTOCKCLI" not in os.environ:
    os.environ["ODYNSTOCKCLI"] = BUILDDIR + '/src/odynstock-cli' + EXEEXT

if EXEEXT == ".exe" and "-win" not in opts:
    # https://github.com/odynstock/odynstock/commit/d52802551752140cf41f0d9a225a43e84404d3e9
    # https://github.com/odynstock/odynstock/pull/5677#issuecomment-136646964
    print("Win tests currently disabled by default.  Use -win option to enable")
    sys.exit(0)

if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_ODYNSTOCKD == 1):
    print("No rpc tests to run. Wallet, utils, and odynstockd must all be enabled")
    sys.exit(0)
...
class RPCCoverage(object):
    ...
    commands per `odynstock-cli help` (`rpc_interface.txt`).
